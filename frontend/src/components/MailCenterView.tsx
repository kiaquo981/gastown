...
export default function MailCenterView() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0e17] text-white/[0.87] font-mono">
      {/* Stats Bar */}
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="border-b border-white/5 bg-[#080b14] px-6 py-4">
      ...
      {/* Tab Bar */}
      <div className="border-b border-white/5 bg-[#080b14] px-6 flex">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2.5 text-xs border-b-2 transition-colors ${tab === t.key ? 'border-blue-400 text-blue-400 bg-blue-500/5' : 'border-transparent text-white/40 hover:text-white/60'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex" style={{ height: 'calc(100vh - 120px)' }}>
        ...
        {(tab === 'inbox' || tab === 'sent') && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="w-64 border-r border-white/5 bg-[#080b14] overflow-y-auto flex-shrink-0">
            <div className="p-3 border-b border-white/5 text-[10px] text-white/30 uppercase tracking-widest">Mailboxes</div>
            {sorted.map(mb => (
            ...
            {(tab === 'inbox' || tab === 'sent') && (
              <motion.div key="inbox" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col">
                {selBox ? (
                  <>
                    {/* Toolbar */}
                    <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#080b14]">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => markAllRead(selBox)} className="px-2 py-1 text-[10px] bg-white/5 text-white/40 border border-white/10 rounded-none hover:bg-white/10 transition-colors">
                          Mark All Read
                        </button>
                        {curMb && (
                          <button onClick={() => toggleDnd(selBox, !curMb.dnd)}
                            className={`px-2 py-1 text-[10px] border rounded-none transition-colors ${curMb.dnd ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-white/5 text-white/40 border-white/10 hover:bg-white/10'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                      {filtered.map(msg => (
                        <motion.div key={msg.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-white/60 truncate" style={{ maxWidth: 120 }}>{tab === 'sent' ? msg.to : msg.from}</span>
                              <span className={`text-xs truncate flex-1 ${msg.read ? 'text-white/40' : 'text-white/80 font-semibold'}`}>{msg.subject}</span>
                            </div>
                            <div className="text-[10px] text-white/25 mt-0.5 truncate">{trunc(msg.body, 80)}</div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                            <span className={`text-[9px] uppercase ${TYPE_CLR[msg.type] || 'text-white/30'}`}>{msg.type}</span>
            ...
            {tab === 'mailboxes' && (
              <motion.div key="mb" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={runCleanup} className="px-2 py-1 text-[10px] bg-white/5 text-white/40 border border-white/10 rounded-none hover:bg-white/10 transition-colors">Cleanup</button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                  {sorted.map(mb => (
                    <motion.div key={mb.workerId} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                      className="bg-[#080b14] border border-white/5 rounded-none p-4 hover:border-white/10 transition-colors cursor-pointer"
            ...
            {tab === 'broadcast' && (
              <motion.div key="bc" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 max-w-xl">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">Subject</label>
                    <input value={bSubj} onChange={e => setBSubj(e.target.value)} className="w-full bg-[#080b14] border border-white/10 rounded-none px-3 py-2 text-xs text-white/80 focus:border-blue-500/40 focus:outline-none" placeholder="Subject..." />
                  </div>
                  <div>
                    <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">Body</label>
                    <textarea value={bBody} onChange={e => setBBody(e.target.value)} rows={6} className="w-full bg-[#080b14] border border-white/10 rounded-none px-3 py-2 text-xs text-white/80 focus:border-blue-500/40 focus:outline-none resize-none" placeholder="Message..." />
                  </div>
                  <div>
                    <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">Priority</label>
                    <div className="flex gap-2">
                      {(['low', 'normal', 'high', 'critical'] as const).map(p => (
                        <button key={p} onClick={() => setBPri(p)}
                          className={`px-3 py-1.5 text-[10px] border rounded-none transition-colors ${bPri === p ? PRIO[p] : 'bg-white/[0.02] text-white/30 border-white/5'}`}>{p}</button>
                      ))}
                    </div>
                  </div>
            ...
            {tab === 'queue' && (
              <motion.div key="q" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4">
                ...
                <div className="space-y-2">
                  {queueWorkers.map(qw => (
                    <div key={qw.id} className="bg-[#080b14] border border-white/5 rounded-none p-4 flex items-center justify-between">
                      <div>
        ...
        <AnimatePresence>
          {selMsg && (tab === 'inbox' || tab === 'sent') && (
            <motion.div key="det" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
              className="w-96 border-l border-white/5 bg-[#080b14] overflow-y-auto flex-shrink-0">
              <div className="p-4 border-b border-white/5 flex items-center justify-between">
                <span className="text-xs text-white/50">Detail</span>
                <button onClick={() => setSelMsg(null)} className="text-white/30 hover:text-white/60 text-sm">x</button>
              </div>
              <div className="p-4 space-y-4">
                ...
                <div className="flex gap-4">
                  <div><div className="text-[10px] text-white/25 uppercase tracking-widest mb-1">From</div><div className="text-xs text-white/60">{selMsg.from}</div></div>
                  <div><div className="text-[10px] text-white/25 uppercase tracking-widest mb-1">To</div><div className="text-xs text-white/60">{selMsg.to}</div></div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-xs text-white/60 whitespace-pre-wrap leading-relaxed bg-white/[0.02] border border-white/5 rounded-none p-3">{selMsg.body}</div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setCSubj(`Fwd: ${selMsg.subject}`); setCBody(`\n\n--- Forwarded ---\nFrom: ${selMsg.from}\n\n${selMsg.body}`); setShowCompose(true); }}
                    className="px-3 py-1.5 text-[10px] bg-white/5 text-white/40 border border-white/10 rounded-none hover:bg-white/10 transition-colors">Forward</button>
                </div>
              </div>
            </motion.div>
          )}
        ...
        {showCompose && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-[#0a0e17] border border-white/10 rounded-none w-full max-w-lg p-6 space-y-4" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-white/70">Compose Message</h3>
                <button onClick={() => setShowCompose(false)} className="text-white/30 hover:text-white/60 text-sm">x</button>
              </div>
              <div>
                <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">To</label>
                <select value={cTo} onChange={e => setCTo(e.target.value)} className="w-full bg-[#080b14] border border-white/10 rounded-none px-3 py-2 text-xs text-white/80 focus:border-blue-500/40 focus:outline-none">
              ...
              <div>
                <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">Subject</label>
                <input value={cSubj} onChange={e => setCSubj(e.target.value)} className="w-full bg-[#080b14] border border-white/10 rounded-none px-3 py-2 text-xs text-white/80 focus:border-blue-500/40 focus:outline-none" placeholder="Subject..." />
              </div>
              <div>
                <label className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">Body</label>
                <textarea value={cBody} onChange={e => setCBody(e.target.value)} rows={5} className="w-full bg-[#080b14] border border-white/10 rounded-none px-3 py-2 text-xs text-white/80 focus:border-blue-500/40 focus:outline-none resize-none" placeholder="Message body..." />
              </div>
              <div className="flex gap-4">
                  ...
                  <div className="flex gap-1">
                    {(['low', 'normal', 'high', 'critical'] as const).map(p => (
                      <button key={p} onClick={() => setCPri(p)}
                        className={`flex-1 px-2 py-1.5 text-[10px] border rounded-none transition-colors ${cPri === p ? PRIO[p] : 'bg-white/[0.02] text-white/30 border-white/5'}`}>{p}</button>
                    ))}
                  </div>
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex gap-1">
                    {(['info', 'task', 'escalation', 'alert'] as const).map(t => (
                      <button key={t} onClick={() => setCType(t)}
                        className={`flex-1 px-2 py-1.5 text-[10px] border rounded-none transition-colors ${cType === t ? 'bg-white/10 text-white/70 border-white/20' : 'bg-white/[0.02] text-white/30 border-white/5'}`}>{t}</button>
                    ))}
                  </div>
                </div>
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setShowCompose(false)} className="px-4 py-2 text-xs text-white/40 border border-white/10 rounded-none hover:bg-white/5 transition-colors">Cancel</button>
      ...
      {/* Loading Overlay */}
      {loading && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-[#0a0e17]/80 pointer-events-none">
          <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }} className="text-xs text-white/30 font-mono">
            Loading mail center...
          </motion.div>
        </div>
      )}
