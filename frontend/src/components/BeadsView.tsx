...
export default function BeadsView() {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a] font-mono text-white/90">
      {/* Header */}
      <div className="flex-none px-6 py-4 border-b border-white/5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-white/40">Atomic Work Units</span>
        ...
        {stats && (
          <div className="flex gap-3 mb-4 overflow-x-auto">
            <div className="px-3 py-2 bg-[#0d1117] border border-white/5 min-w-[100px]">
              <div className="text-[10px] text-white/40 uppercase">Total</div>
              <div className="text-lg font-bold">{stats.total}</div>
            </div>
            {STATUS_COLUMNS.map(col => (
              <div key={col.key} className="px-3 py-2 bg-[#0d1117] border border-white/5 min-w-[90px]">
                <div className="text-[10px] text-white/40 uppercase">{col.label}</div>
            ...
            <div className="px-3 py-2 bg-[#0d1117] border border-white/5 min-w-[100px]">
              ...
              <div className="text-lg font-bold text-emerald-400">{typeof stats.velocity === 'object' ? (stats.velocity as any)?.avg_per_week ?? 0 : stats.velocity ?? 0}<span className="text-xs text-white/30">/wk</span></div>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <input
            ...
            className="px-3 py-1.5 text-xs bg-[#0d1117] border border-white/10 text-white/90 placeholder-white/30 w-64 focus:outline-none focus:border-white/20"
          />
          <select value={filterBu} onChange={e => setFilterBu(e.target.value)} className="px-2 py-1.5 text-xs bg-[#0d1117] border border-white/10 text-white/70">
            <option value="all">All BUs</option>
            {allBus.map(bu => <option key={bu} value={bu}>{bu}</option>)}
          </select>
          <select value={filterRig} onChange={e => setFilterRig(e.target.value)} className="px-2 py-1.5 text-xs bg-[#0d1117] border border-white/10 text-white/70">
            <option value="all">All Rigs</option>
            {allRigs.map(rig => <option key={rig} value={rig}>{rig}</option>)}
          </select>
          <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)} className="px-2 py-1.5 text-xs bg-[#0d1117] border border-white/10 text-white/70">
          ...
          <select value={filterTier} onChange={e => setFilterTier(e.target.value)} className="px-2 py-1.5 text-xs bg-[#0d1117] border border-white/10 text-white/70">
            <option value="all">All Tiers</option>
            {allTiers.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <button onClick={() => { fetchBeads(); fetchStats(); }} className="px-2 py-1.5 text-xs text-white/40 hover:text-white/70 border border-white/10 bg-[#0d1117]">
            Refresh
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-full text-white/30 text-sm">Loading beads...</div>
        ) : (
            ...
            {STATUS_COLUMNS.map(col => (
              <div key={col.key} className="flex flex-col w-[280px] min-w-[280px]">
                ...
                <div className="flex items-center gap-2 px-3 py-2 mb-2 border-b border-white/5">
                  ...
                  <span className="text-xs font-bold text-white/70 uppercase">{col.label}</span>
                  <span className="text-[10px] text-white/30 ml-auto">{beadsByStatus[col.key].length}</span>
                </div>
                {/* Cards */}
                <div className="flex-1 overflow-y-auto space-y-2 pr-1 pb-4">
                  {beadsByStatus[col.key].map(bead => (
                    <button
                      ...
                      className={`w-full text-left p-3 bg-[#0d1117] border border-white/5 hover:border-white/10 transition-colors ${
                        selectedBead?.id === bead.id ? 'border-white/20 bg-white/[0.03]' : ''
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <span className="text-xs text-white/80 leading-tight line-clamp-2">{bead.title}</span>
                        {bead.priority && (
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-white/30">
                        ...
                        {bead.bu && <span className="px-1 py-0.5 bg-white/5 border border-white/5">{bead.bu}</span>}
                        {bead.tier && <span className="px-1 py-0.5 bg-white/5 border border-white/5">{bead.tier}</span>}
                      </div>
                  ...
                  {beadsByStatus[col.key].length === 0 && (
                    <div className="text-center py-8 text-white/15 text-[10px] uppercase">Empty</div>
                  )}
                </div>
              </div>
            ))}
      ...
      {selectedBead && (
        <div className="fixed inset-y-0 right-0 w-[420px] bg-[#0a0a0a] border-l border-white/5 z-50 flex flex-col overflow-hidden shadow-2xl">
          {/* Detail header */}
          <div className="flex-none px-5 py-4 border-b border-white/5 flex items-center justify-between">
            <span className="text-xs font-bold text-white/70 uppercase">Bead Detail</span>
            <button onClick={() => { setSelectedBead(null); setEditingStatus(null); }} className="text-white/30 hover:text-white/60 text-sm">X</button>
          </div>
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {/* ID */}
            <div>
              <div className="text-[10px] text-white/30 uppercase mb-1">ID</div>
              <div className="text-xs font-mono text-white/60">{selectedBead.id}</div>
            </div>
            {/* Title */}
            <div>
              <div className="text-[10px] text-white/30 uppercase mb-1">Title</div>
              <div className="text-sm text-white/90">{selectedBead.title}</div>
            </div>
            {/* Description */}
            {selectedBead.description && (
              <div>
                <div className="text-[10px] text-white/30 uppercase mb-1">Description</div>
                <div className="text-xs text-white/60 whitespace-pre-wrap">{selectedBead.description}</div>
              </div>
            )}
            {/* Status */}
            <div>
              ...
              {editingStatus !== null ? (
                <div className="flex flex-wrap gap-1">
                  ...
                  <button onClick={() => setEditingStatus(null)} className="px-2 py-1 text-[10px] text-white/30 border border-white/10">Cancel</button>
                </div>
              ) : (
                <button
            ...
            {/* Priority */}
            <div>
              <div className="text-[10px] text-white/30 uppercase mb-1">Priority</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {selectedBead.bu && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">BU</div>
                  <div className="text-xs text-white/60">{selectedBead.bu}</div>
                </div>
              )}
              {selectedBead.rig && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Rig</div>
                  <div className="text-xs text-white/60">{selectedBead.rig}</div>
                </div>
              )}
              {selectedBead.tier && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Tier</div>
                  <div className="text-xs text-white/60">{selectedBead.tier}</div>
                </div>
              )}
              {selectedBead.skill && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Skill</div>
                  <div className="text-xs text-white/60">{selectedBead.skill}</div>
                </div>
              )}
              {selectedBead.executorType && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Executor</div>
                  <div className="text-xs text-white/60">{selectedBead.executorType}</div>
                </div>
              )}
              {selectedBead.assignee && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Assignee</div>
                  <div className="text-xs text-white/60">{selectedBead.assignee}</div>
                </div>
              )}
            </div>
            {/* Labels */}
            {selectedBead.labels && selectedBead.labels.length > 0 && (
              <div>
                <div className="text-[10px] text-white/30 uppercase mb-1">Labels</div>
                <div className="flex flex-wrap gap-1">
                  {selectedBead.labels.map(l => (
                    <span key={l} className="px-1.5 py-0.5 text-[10px] bg-white/5 border border-white/10 text-white/50">{l}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-white/5">
              {selectedBead.createdAt && (
                <div>
                  ...
                  <div className="text-[10px] text-white/40">{new Date(selectedBead.createdAt).toLocaleString()}</div>
                </div>
              )}
              {selectedBead.updatedAt && (
                <div>
                  <div className="text-[10px] text-white/30 uppercase mb-1">Updated</div>
                  <div className="text-[10px] text-white/40">{new Date(selectedBead.updatedAt).toLocaleString()}</div>
                </div>
              )}
            </div>
          </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-[520px] bg-[#0d1117] border border-white/10 p-6">
            <div className="flex items-center justify-between mb-5">
              <span className="text-sm font-bold text-white/80">New Bead</span>
              <button onClick={() => setShowCreateModal(false)} className="text-white/30 hover:text-white/60 text-sm">X</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-[10px] text-white/30 uppercase mb-1">Title *</label>
                <input
                  type="text"
                  value={newTitle}
                  onChange={e => setNewTitle(e.target.value)}
                  placeholder="What needs to be done?"
                  className="w-full px-3 py-2 text-xs bg-[#0a0a0a] border border-white/10 text-white/90 placeholder-white/20 focus:outline-none focus:border-white/20"
                />
              </div>
              <div>
                <label className="block text-[10px] text-white/30 uppercase mb-1">Description</label>
                <textarea
                  value={newDescription}
                  onChange={e => setNewDescription(e.target.value)}
                  rows={3}
                  placeholder="Details, context, acceptance criteria..."
                  className="w-full px-3 py-2 text-xs bg-[#0a0a0a] border border-white/10 text-white/90 placeholder-white/20 focus:outline-none focus:border-white/20 resize-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">Priority</label>
                  <select value={newPriority} onChange={e => setNewPriority(e.target.value as BeadPriority)} className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70">
                ...
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">Executor Type</label>
                  <input
                    type="text"
                    value={newExecutorType}
                    onChange={e => setNewExecutorType(e.target.value)}
                    placeholder="agent, human, hybrid"
                    className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70 placeholder-white/20 focus:outline-none"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">BU</label>
                  <input type="text" value={newBu} onChange={e => setNewBu(e.target.value)} placeholder="Business Unit" className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70 placeholder-white/20 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">Rig</label>
                  <input type="text" value={newRig} onChange={e => setNewRig(e.target.value)} placeholder="Rig name" className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70 placeholder-white/20 focus:outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">Skill</label>
                  <input type="text" value={newSkill} onChange={e => setNewSkill(e.target.value)} placeholder="Required skill" className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70 placeholder-white/20 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-[10px] text-white/30 uppercase mb-1">Tier</label>
                  <input type="text" value={newTier} onChange={e => setNewTier(e.target.value)} placeholder="S, A, B, C" className="w-full px-2 py-1.5 text-xs bg-[#0a0a0a] border border-white/10 text-white/70 placeholder-white/20 focus:outline-none" />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowCreateModal(false)} className="px-3 py-1.5 text-xs text-white/40 border border-white/10 hover:bg-white/5">Cancel</button>
              <button
                onClick={createBead}
