...
export async function spawnPolecat(options: PolecatSpawnOptions): Promise<PolecatResult> {
  ...
  try {
    ...
    // For code-type skills, attempt worktree + PR → auto-enqueue into Refinery
    ...
    if (isCodeSkill) {
      ...
      if (worktreePath) {
        ...
        // Auto-enqueue into Refinery for quality gates + merge
        try {
          const mergeItem = refinery.enqueue({
            branch,
            author: `polecat-${polecatId}`,
            title: options.bead.title,
            beadId: options.bead.id,
            priority: options.tier === 'S' ? 0 : options.tier === 'A' ? 10 : 50,
          });

          // Fire-and-forget: run gates + attempt auto-merge
          void (async () => {
            const gateResult = await refinery.runGates(mergeItem.id);
            if (gateResult.passed && refinery.canFastPath(mergeItem.id)) {
              await refinery.fastPathMerge(mergeItem.id);
              log.info({ mergeItemId: mergeItem.id, beadId: options.bead.id }, 'Auto-merged via fast-path');
            }
          })();
        } catch (err) {
          log.warn({ err, polecatId }, 'Failed to enqueue into refinery');
        }
      }
    }
